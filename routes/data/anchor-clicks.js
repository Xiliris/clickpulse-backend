  ...
  try {
    ...
    if (!startDate || !endDate) {
      [rows] = await database.query(
        "SELECT * FROM anchors WHERE domain = ? ORDER BY date DESC",
        [authorized.domain]
      );
    ...
    [rows] = await database.query(
      "SELECT elementId, content, SUM(clicks) AS clicks FROM anchors WHERE domain = ? AND date BETWEEN ? AND ? GROUP BY elementId, content ORDER BY clicks DESC",
    ...
    console.error("Error during fetching anchors:", error.message);
